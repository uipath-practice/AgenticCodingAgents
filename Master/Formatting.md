...
- **Quotations and citations.** A blockquote never shares a two-column row with an
  image. Put the quote in its own full-width block, with the image above or below it.
  Quotes are set off from the body deliberately; squeezing one into a column beside a
  screenshot fights that and reads as a caption for the picture.

### Quotations

Use a blockquote, full width, with the attribution as the last line:
...
If a screenshot or illustration belongs with the quote, place it on its own line
before or after the blockquote — never inside a `[[[ ... ]]]` block with it.
