...
Use a blockquote with the attribution as the last line:

```markdown
> The art of keeping your projects organized is rooted in habits — and habits are
> nurtured through consistent practice.
>
> — *Generated by a wise ancient LLM*
```

When an illustration accompanies the quote, pair them in a two-column block with the
**image on the left** and the quote on the right:

```markdown
[[[
![Illustration of a friendly robot presenting a checklist](step-slug.images/3-robot.png)
|30|
> The art of keeping your projects organized is rooted in habits — and habits are
> nurtured through consistent practice.
>
...
Use `|30|` for these decorative illustrations. They are small (typically ~340px wide),
so a wider column upscales and blurs them. Omit any `width=` attribute — the
`.img-cols > div img` rule already sizes images inside columns.

A quote with no accompanying image stays full width.
